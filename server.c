...
static const int serverID = 0; // Static constant to identify the server (0 for main, 1 for mirror)
...
// Function to determine whether a connection should be accepted.
int shouldAcceptConnection(int connectionNumber, int serverID) {
    if ( (connectionNumber >= 0) && (connectionNumber <= 3) ){
        return 1;
    } 
    else if ( (connectionNumber > 7) && ((connectionNumber % 2) == serverID) ) {
        return 1;
    } else {
        return 0;
    }
        ...
        if (entry->d_type == DT_REG) {
            ...
            if (strcmp(entry->d_name, filename) == 0) {
                ...
                if (stat(full_path, &statbuf) == 0) {
                    ...
                    strftime(formattedTime, sizeof(formattedTime), "%a %d %b %Y %I:%M:%S %p %Z", timeinfo); //weekday, day of month, monthname, year, hour, min, sec, am/pm, timezone
...
void serverLoop(int server_socket, int serverID) {
    while (1) {
        ...
        // Check if the server should accept this connection based on serverID and connection number
        if (!shouldAcceptConnection(connectionNumber, serverID)) {
            // If not accepted, send 'DECLINED' message to client and close the socket
            send(client_socket, "DECLINED", 8, 0);
            close(client_socket);
            continue;
        }
        // If accepted, send 'ACCEPTED' message to client
        send(client_socket, "ACCEPTED", 8, 0);
